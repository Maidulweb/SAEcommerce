           ...
           @php
               foreach ($items as $key => $value) {
                  ...
                  if (array_keys($lastkey)[0] == 'category') {
                    ...
                    $products = \App\Models\Product::where('category_id', $lastkey['category'])->take(5)->orderBy('id', 'DESC')->get();
                  }elseif(array_keys($lastkey)[0] == 'sub_category'){
                    $category = \App\Models\SubCategory::find($lastkey['sub_category']);
                    $products = \App\Models\Product::where('sub_category_id', $lastkey['sub_category'])->take(5)->orderBy('id', 'DESC')->get();
                  }else{
                    $category = \App\Models\ChildCategory::find($lastkey['child_category']);
                    $products = \App\Models\Product::where('child_category_id', $lastkey['child_category'])->take(5)->orderBy('id', 'DESC')->get();
                  }
                 } 
           @endphp
